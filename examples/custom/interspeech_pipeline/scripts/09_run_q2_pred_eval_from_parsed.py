...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Compute prediction-aware Q2 metrics using predicted region IDs and GT tuples.")
    parser.add_argument("--pred-meta-json", required=True, help="Dataset JSON aligned with generation rows and predicted prompt1_output.")
    parser.add_argument("--gt-meta-json", required=True, help="Ground-truth dataset JSON containing GT prompt1_output and gt_prompt2.")
...
def main():
    args = parse_args()
    pred_meta_rows = json.loads(Path(args.pred_meta_json).read_text(encoding="utf-8"))
    gt_meta_rows = json.loads(Path(args.gt_meta_json).read_text(encoding="utf-8"))
    raw_jsonl = Path(args.raw_result_jsonl)
    ver_dir = Path(args.verifier_output_dir)
    out_json = Path(args.output_json)

    if not isinstance(pred_meta_rows, list) or not isinstance(gt_meta_rows, list):
        raise ValueError("Both pred-meta-json and gt-meta-json must be JSON arrays.")

    gt_by_key = {}
    gt_ids_by_sample = {}
    for row in gt_meta_rows:
        if not isinstance(row, dict):
            continue
        sample_id = norm(row.get("sample_id"))
        gt_prompt_ids = parse_prompt_ids(row.get("prompt1_output"))
        gt_slots = parse_indexed_tuple_text(row.get("gt_prompt2"))
        gt_ids_by_sample[sample_id] = set(gt_prompt_ids)
        for slot in (1, 2, 3):
            if slot > len(gt_prompt_ids):
                continue
            region_id = gt_prompt_ids[slot - 1]
            slot_data = gt_slots.get(slot, {})
            gt_by_key[(sample_id, region_id)] = {
    ...
    with raw_jsonl.open("r", encoding="utf-8") as f:
        for idx, line in enumerate(f):
            ...
            try:
                ...
                continue
            pred_meta = pred_meta_rows[idx] if idx < len(pred_meta_rows) and isinstance(pred_meta_rows[idx], dict) else {}
            sample_id = norm(obj.get("sample_id")) or norm(pred_meta.get("sample_id"))
            prompt1_output = norm(obj.get("prompt1_output")) or norm(pred_meta.get("prompt1_output"))
            pred_prompt_ids = parse_prompt_ids(prompt1_output)
            parsed = parse_predicted_sections(get_response(obj), pred_prompt_ids)
            if len(pred_prompt_ids) >= 3:
                for slot in (1, 2, 3):
                    region_id = pred_prompt_ids[slot - 1]
                    fields = parsed.get(slot, {})
                    pred_rows.append({
                        "sample_id": sample_id,
                        "slot": slot,
                        "region_id": region_id,
                        "has_tuple": bool(fields),
                    })
                    if fields:
                        pred_fields_by_key[(sample_id, region_id)] = fields
                        pred_en_by_key[(sample_id, region_id)] = norm(fields.get("En"))
    ...
    overlap_rows_with_verifier = 0
    consistency_rows = 0
    extractable = {"T": 0, "F": 0, "P": 0}
    agree = {"T": 0, "F": 0, "P": 0}
    sample_region_correct = {}
    overlap_verified_ids_by_sample = {}

    for row in pred_rows:
        sample_id = row["sample_id"]
        region_id = row["region_id"]
        gt = gt_by_key.get((sample_id, region_id))
        pred_ver = verifier_by_key.get((sample_id, region_id), {})
        pt = pred_ver.get("time")
        pf = pred_ver.get("frequency")
        pp = pred_ver.get("phonetic")
        ...
        sample_region_correct[(sample_id, region_id)] = (
            pt == gt["time"] and pf == gt["frequency"] and pp == gt["phonetic"]
        )
        overlap_verified_ids_by_sample.setdefault(sample_id, set()).add(region_id)

    n_regions_total = len(pred_rows)
    n_regions = overlap_rows_with_verifier
    ...
    coverage_p = (extractable["P"] / consistency_rows) if consistency_rows else 0.0
    ...
    coverage_avg = mean([coverage_t, coverage_f, coverage_p]) if consistency_rows else 0.0
    ...
    for sample_id, gt_region_ids in gt_ids_by_sample.items():
        ...
        if len(pred_verified_overlap) != 3:
            continue
        if pred_verified_overlap != gt_region_ids:
            continue
        eligible_samples += 1
        if all(sample_region_correct.get((sample_id, region_id), False) for region_id in gt_region_ids):
            sample_all9_correct += 1

    caption = caption_scores(gt_caps, pred_caps)

    metrics = {
        "num_regions_total": n_regions_total,
        "num_regions_with_verifier": field_rows_with_verifier,
        "num_overlap_regions_total": overlap_rows_total,
        ...
        "consistency": {
            "ConsScore": consscore,
            "RowsWithVerifier": consistency_rows,
